...
\begin{figure}[h]
	\includegraphics[width=\textwidth, height=8cm]{images/gui/CasosUso/Contador}
...
\begin{figure}[h]
	\includegraphics[width=\textwidth, height=10cm]{images/gui/CasosUso/Caja}
...
Un diagrama de clases ayuda a describir la vista de diseño estática de un sistema. En este diagrama podemos visualizar el tipo de relación que presenta cada clase y el como interactua con las demás.
A grandes rasgos podemos observar la composición una cuenta dentro del sistema, la generalización de áreas de servicio y los requisitos necesarios para generar un pago, así como también los posibles componentes que pueden interactuar con el pago. Figura \ref{fig:DiagramaClases}
...
Este proceso tiene como objetivo registrar un usuario nuevo, en la siguiente imagen se observa que el usuario ingresa al sistema en la pantalla Registrar Usuario, en esta pantalla ingresa la información solicitada, cuando se hace la petición los datos pasan por el controlador llamado Action en Struts 2, se hace una validación de estos datos cuando pasan a la capa del negocio, si cumple con estas validaciones, el sistema envía la información a la capa de persistencia para guardarla, si esto ocurre normalmente envía una respuesta pasando por las capas anteriormente mencionadas hasta llegar al controlador quien envía el mensaje que indica que el usuario se ha registrado correctamente. Figura \ref{fig:SeqRegistrarUsuario}
\begin{figure}[h]
...
\subsection{Visualizar servicios}
El objetivo de este diagrama es detallar el mecanismo otorgado al usuario para poder visualizar el catálogo de servicios usados actualmente en la unidad académica, el usuario ingresa a la pantalla Visualizar Servicios, por su parte el sistema invoca la capa del controlador verifica la sesión del usuario. Si el usuario tiene sesión la petición se manda a la capa de negocio quien por medio del uso de una referencia a la capa de persistencia busca todos los campos que representan el catálogo de servicios, una vez realizado esto responde a la capa de negocio y finalmente pasa los datos recibidos por la capa del controlador para renderizar la lista de servicios en una vista. Figura \ref{fig:SeqVisualizarServicios}
\begin{figure}[h]
...
El objetivo de este diagrama es mostrar de forma más detallada el proceso de realización de pagos, el cual comienza cuando un usuario desea adquirir un servicio disponible en en el catálogo de pagos, después de seleccionar el servicio el sistema redirige a la pantalla Adjuntar comprobante para poder insertar el comprobante y el folio del comprobante que desea enviar para su posterior aprobación o rechazo por parte del cajero, cuando el usuario hace la petición los datos llegan a la capa del controlador quien direcciona los datos a la capa del negocio, esta capa válida que el folio no sea exista en la base de datos y que el día de emisión no sea el mismo, si esto se cumple, finalmente envía los datos a la capa de persistencia la cual deja el pago en el sistema para su posterior evaluación por parte del cajero. Figura \ref{fig:SeqRealizarPago}
\begin{figure}[h]
...
El objetivo de este proceso es otorgarle al cajero la capacidad autorizar pagos o rechazar pagos, como primer paso el cajero con sesión activa se dirige a la pantalla para visualizar el comprobante de pago, si desea autorizar el pago envía la petición y la recibe el controlador con la referencia del pago que ha solicitado aprobar el usuario, si el pago existe pasa a la capa de negocio quien invoca a la capa de persistencia para poder actualizar el pago con la referencia del pago, si todo sucede normalmente, la capa de persistencia responde a la capa de negocio y la de negocio a Action quien finalmente redirige a la página de autorización de pagos. Figura \ref{fig:SeqAprobarPago}
...
El objetivo de este diagrama es detallar el mecanismo otorgado al usuario para poder visualizar pagos, en la primer parte el action recibe el id del pago que se desea visualizar, en la capa de negocio valida que el pago exista y si existe busca por medio del id el archivo de pago para responder a las capas invocadas anteriormente cuando regresa al controlador este renderiza los bytes que representan los archivos para mostrarlos en la pantalla Visualizar Comprobante de Pago. Figura \ref{fig:SeqVisualizarPago}
\begin{figure}[h]
...
El objetivo de este diagrama es detallar el mecanismo otorgado al usuario para poder visualizar pagos que han sido aprobados por caja, en la primer parte el action recibe el id del pago que se desea visualizar, en la capa de negocio valida que el pago exista y si existe busca por medio del id el archivo de pago para responder a las capas invocadas anteriormente cuando regresa al controlador este renderiza los bytes que representan los archivos para mostrarlos en la pantalla Visualizar Comprobante de Pago, el campo estado del pago representa si un pago ha sido aprobado o rechazado. Figura \ref{fig:SeqVisualizarPagosAprobados}
\begin{figure}[h]
...
El objetivo de este diagrama es detallar el mecanismo otorgado al cajero para poder visualizar los comprobantes de pago emitidos por el subsistema SIG@, en la primer parte el action recibe el id del pago que se desea visualizar el cual está asociado por un comprobante SIG@ puesto que todo pago emitido hacia la unidad académica debe generar un comprobante SIG@, en la capa de negocio valida que el pago exista y que tenga adjunto un comprobante SIG@, si existe busca por medio del id el archivo de pago para responder a las capas invocadas anteriormente cuando regresa al controlador este renderiza los bytes que representan los archivos para mostrarlos en la pantalla Visualizar Comprobante de SIG@. Figura \ref{fig:SeqVisualizarSIGA}
\begin{figure}[h]
...
Como podemos ver de los diagramas de secuencia anteriores el proceso que lleva cada cada consiste en la comunicación de interfaz gráfica-action-business-Data Access Object permitiendo así un diagrama de secuencia general para cada proceso.
\begin{figure}[h]
...
\subsection{Maquina de estados pagos}
La máquina de la siguiente figura muestra los posibles estados de un pago, por una parte cuando se emite un pago este pasa al estado pendiente para que la caja pueda visualizarlo y modificar su estado, si la caja autoriza el pago entonces debe adjuntar una archivo SIG@, cuando esto sucede el estado del pago se modifica a aprobado, el pago de la caja cumple con que está autorizado, tienen asociado su pago SIG@ y además ha sido procesado en un corte de caja su estado será adjuntado, por otro lado, si el pago es rechazado por  caja entonces modifica su estado y lo mantendrá como pendiente hasta que el cajero lo autorice y por lo tanto lo procese. Figura \ref{fig:MaquinaEstados}
\begin{figure}[h]
	\includegraphics[width=\textwidth]{images/gui/Estados}
	\caption{Maquina de estados pagos}
